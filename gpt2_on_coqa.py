...
for example in val_data_raw:
    ...
    for q, a in zip(questions, answers):
        ...
        if answer_start == -1:
            continue
        flattened["id"].append(str(uuid.uuid4()))
        flattened["context"].append(context)
        flattened["question"].append(q)
        flattened["answer_text"].append(a)
        flattened["answer_start"].append(answer_start)

val_data = Dataset.from_dict(flattened)
...
with open("logs/gpt2_on_coqa.txt", "w") as f:
